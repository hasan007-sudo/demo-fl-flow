...
export async function POST(request: Request) {
  try {
    ...
    if (agentType !== 'english_tutor' && agentType !== 'interview_preparer') {
      return NextResponse.json(
        { error: 'agentType must be either "english_tutor" or "interview_preparer"' },
        { status: 400 }
      );
    }
    ...
    // Fetch the user's most recent questionnaire response based on agent type
