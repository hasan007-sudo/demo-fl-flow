...
function InterviewPreparerSessionContent() {
      ...
      try {
        ...
        const sessionResponse = await fetch('/api/sessions', {
          ...
          body: JSON.stringify({
            ...
            ...(responseId && { interviewResponseId: responseId }),
          }),
        });
  ...
  const handleLeave = () => {
    ...
    if (sessionId) {
      router.push(`/survey?sessionId=${sessionId}&agentType=interview_preparer`);
    } else {
      router.push('/');
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 text-gray-900 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600 mx-auto mb-4"></div>
          <p className="text-gray-700">Connecting to your interviewer...</p>
        </div>
      </div>
    );
  }
...
export default function InterviewPreparerSessionPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 text-gray-900 flex items-center justify-center">
    ...
    >
      <InterviewPreparerSessionContent />
    </Suspense>
  );
}
